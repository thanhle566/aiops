"""
InfluxdbProvider is a class that provides a way to read data from influxdb.
"""
...
class InfluxdbProviderAuthConfig:
    username: str = dataclasses.field(
        metadata={"required": True, "description": "Influxdb username"}
    )
    password: str = dataclasses.field(
        metadata={"required": True, "description": "Influxdb password", "sensitive": True}
    )
    ...
    database: str | None = dataclasses.field(
        metadata={"required": False, "description": "Influxdb database name"}, default=None
    )
    port: int | None = dataclasses.field(
        metadata={"required": False, "description": "Influxdb port"}, default=3306
    )


class InfluxdbProvider(BaseProvider):
    """Enrich alerts with data from influxdb."""
    ...
    def __generate_client(self):
        """
        Generates a influxdb client.

        Returns:
            influxdb.connector.CinfluxdbConnection: influxdb Client
        """
        client = influxdb.connector.connect(
    ...
    def dispose(self):
        try:
            ...
            self.logger.exception("Error closing influxdb connection")

    def validate_config(self):
        """
        Validates required configuration for influxdb's provider.
        """
        self.authentication_config = InfluxdbProviderAuthConfig(
            **self.config.authentication
        )

    def _notify(self, **kwargs):
        """
        For influxdb there is no difference if we're querying data or we want to make an impact.
    ...
    def _query(
        ...
        """
        Executes a query against the Influxdb database.
...
if __name__ == "__main__":
    ...
    influxdb_provider = InfluxdbProvider(context_manager, "influxdb-prod", config)
    results = influxdb_provider.query(query="SELECT MAX(datetime) FROM demo_table LIMIT 1")
    print(results)
