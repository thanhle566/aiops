      ...
      {specialNodeCheck && (
        ...
        >
          <div className={`flex flex-col items-center justify-center`}>
            {type === "start" && (
              <MdNotStarted className="size-20 bg-[#2C4A4B] text-white rounded-full font-bold mb-2" />
            )}
            {type === "end" && (
              <GoSquareFill className="size-20 bg-[#2C4A4B] text-white rounded-full font-bold mb-2" />
            )}
            {["threshold", "assert", "foreach"].includes(type) && (
              <div
                className={`border-2 ${
                  id === selectedNode ? "border-[#2C4A4B]" : "border-stone-400"
                }`}
              >
                {id.includes("end") ? (
