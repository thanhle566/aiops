...
export const GroupedRow = ({
  ...
  if (row.getIsGrouped()) {
    ...
    return (
      <>
        {/* Group Header Row */}
        <TableRow className="bg-primary-100 hover:bg-primary-200 cursor-pointer border-t border-primary-300">
          {/* Render a single cell that spans the entire width */}
          <TableCell
            colSpan={row.getVisibleCells().length}
            onClick={() => setIsExpanded(!isExpanded)}
            className="group-header-cell bg-primary-100 group-hover:bg-primary-200"
          >
