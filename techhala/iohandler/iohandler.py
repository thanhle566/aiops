...
import techhala.functions as keep_functions
from techhala.contextmanager.contextmanager import ContextManager
from techhala.step.step_provider_parameter import StepProviderParameter
...
class IOHandler:
    ...
    def extract_keep_functions(self, text):
        ...
        i = 0
        while i < len(text):
            if text[i : i + 5] == "techhala.":
                start = i
                func_start = text.find("(", start)
                if func_start > -1:  # Opening '(' found after "techhala."
                    ...
                    # If no '(' found, increment i to move past "techhala."
                    i += 5
            else:
                i += 1
        return matches
    ...
    def _trim_token_error(self, token):
        ...
        if len(token) > 64:
            try:
                func_name = token.split("techhala.")[1].split("(")[0]
                err = f"techhala.{func_name}(...)"
            except Exception:
                err = token
            finally:
                return err
    ...
    def parse(self, string, safe=False, default="", additional_context=None):
        ...
        if len(tokens) == 0:
            ...
            if isinstance(val, dict):
                ...
                #   method: POST
                #   body:
                #     alert: techhala.json_loads('{{ alert }}')
        ...
        for token in tokens:
            token, escapes = token
            # imagine " techhala.f(..) > 1 and techhala.f(..) <2"
            # so techhala.f already handled, we don't want to handle it again
