    ...
    columnHelper.display({
      ...
      cell: (context) =>
        context.row.original.pre ? (
          <Icon icon={IoCheckmark} size="md"  color="red" />
        ) : (
          <Icon icon={HiMiniXMark} size="md"  color="red" />
        ),
    }),
    columnHelper.display({
      id: "attribute",
    ...
    columnHelper.display({
      ...
      header: () => (
        <div className="flex items-center">
          ...
          <a
            href={`${
              config?.KEEP_DOCS_URL || "https://docs.techhala.com"
      ...
      cell: (context) => (
        <div className="flex flex-wrap">
          {extractNamedGroups(context.row.original.regex).map((attr) => (
            <Badge key={attr}  color="red" size="xs">
              {attr}
            </Badge>
          ))}
        </div>
      ...
      <Table className="border border-gray-200">
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow key={headerGroup.id} className="border-b border-gray-200">
              {headerGroup.headers.map((header) => (
                <TableHeaderCell key={header.id} className="border-r border-gray-200 last:border-r-0">
                  {flexRender(
        ...
        <TableBody>
          {table.getRowModel().rows.map((row) => (
            <>
              <TableRow
                className="hover:bg-slate-100 group cursor-pointer border-b border-gray-200"
                key={row.id}
                onClick={() =>
              ...
              {row.getIsExpanded() && (
                <TableRow className="pl-2.5 border-b border-gray-200">
                  <TableCell colSpan={columns.length} className="border-r border-gray-200 last:border-r-0">
